...
class TokenGenerationTestCase(TestCase):
    ...
    def test_token_expiration(self):
        ...
        expected_expiration = timezone.now() + timedelta(hours=1)
        token_expiration = timezone.datetime.fromtimestamp(token_obj['exp'])

        # Allow for a small time difference due to processing
        self.assertAlmostEqual(token_expiration, expected_expiration, delta=timedelta(seconds=5))

    def test_token_user_details(self):
        token = generate_token(self.user)
        token_obj = AccessToken(token)

        self.assertEqual(token_obj['user_id'], str(self.user.id))
        self.assertEqual(token_obj['email'], self.user.email)
        self.assertEqual(token_obj['first_name'], self.user.first_name)
        self.assertEqual(token_obj['last_name'], self.user.last_name)
...
class AuthAPITestCase(TestCase):
    ...
    def test_register_duplicate_email(self):
        ...
        self.assertIn('already exists', response.data['errors']['email'][0])
    ...
    def test_register_duplicate_userId(self):
        ...
        self.assertIn('already exists', response2.data['errors']['userId'][0])
...
from rest_framework.test import APIClient
from django.contrib.auth import get_user_model
from ..models import Organisation  # Adjust the import path as needed
